...
PortfolioManager follows semantic versions. The current stable release is `0.1.7`; see [CHANGELOG.md](CHANGELOG.md) for release notes.
...
The portable backup downloaded from Settings deliberately excludes Bitvavo credentials and crypto data. Re-enter the credentials and synchronize Bitvavo after restoring it. A direct filesystem database backup is a full system backup and should be protected like a password.

For a new, empty installation, you can restore a downloaded database from **Settings → Database**. The app validates the SQLite backup before restoring it and only enables this action while there is no portfolio data, preventing accidental overwrites. Your local login methods remain in place; reconnect Bitvavo afterwards if needed.
